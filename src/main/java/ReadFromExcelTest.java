...
public class ReadFromExcelTest {
    public static List<Read> readFromExcel(String file) throws IOException {
        ...
        //чтение листа
        try (FileInputStream fis = new FileInputStream("path/to/source.xlsx");
             XSSFWorkbook sourceWb = new XSSFWorkbook(fis)) {
            // ... ваш код
        }

        //копирование и создание новой области
        XSSFSheet sourceSheet = sourceWb.getSheet("Лист1"); // Получаем исходный лист
        XSSFSheet newSheet = sourceWb.cloneSheet(sourceWb.getSheetIndex(sourceSheet));
        newSheet.getSheetName(); // Устанавливаем новое имя для скопированного листа

        //сохранение изменений
        try (FileOutputStream fos = new FileOutputStream("path/to/destination.xlsx")) {
            sourceWb.write(fos);
        }
